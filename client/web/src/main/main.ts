...
import {SwatchRGB, PaletteRGB, fillColor, neutralPalette, accentPalette, controlCornerRadius, designUnit} from '@microsoft/fast-components';
import {parseColorHexRGB, ColorRGBA64} from '@microsoft/fast-colors';
import {HostENV, HostURL} from '../types';
import {logger} from '../utils';

const name = 'hello-world-app';
...
export class MainApplication extends FASTElement {
  ...
  public async connectedCallback() {
    ...
    neutralPalette.setValueFor(this.provider,
      PaletteRGB.from(SwatchRGB.create(neutralPaletteColor.r, neutralPaletteColor.g, neutralPaletteColor.b)));
    const accentPaletteColor: ColorRGBA64 = parseColorHexRGB('#247592');
    accentPalette.setValueFor(this.provider,
  ...
  public async handleConnection() {
    ...
    try {
      this.connect.connect(
        ...
        {reconnectAttempts: 99, reconnectInterval: 5000},
      );
    } catch (err) {
      logger.debug(err);
    }
